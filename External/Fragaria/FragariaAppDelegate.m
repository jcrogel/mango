...
#pragma unused(document, colourWithBlock, string, range, info)
    ...
    // query info
    BOOL willColour = [info[SMLSyntaxWillColour] boolValue];
    NSDictionary *syntaxInfo = info[SMLSyntaxInfo];
...
- (BOOL)fragariaDocument:(id)document shouldColourGroupWithBlock:(BOOL (^)(NSDictionary *, NSRange))colourWithBlock string:(NSString *)string range:(NSRange)range info:(NSDictionary *)info
{
    ...
    // query info
    NSString *group = info[SMLSyntaxGroup];
    NSInteger groupID = [info[SMLSyntaxGroupID] integerValue];
    BOOL willColour = [info[SMLSyntaxWillColour] boolValue];
    NSDictionary *attributes = info[SMLSyntaxAttributes];
